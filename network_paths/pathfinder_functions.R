...
# these are functions to find paths between nodes through the SIGNOR database
...
# convert to SIF, maximal length

function_convert_SIF_three_intermediates <- function(sample_path,colnames_3cols){
...
# convert to SIF, max 2 intermediate nodes

function_convert_SIF_two_intermediates <- function(sample_path,source_nodes,colnames_3cols){
...
# convert to SIF, max 1 intermediate node
function_convert_SIF_one_intermediate <- function(sample_path,source_nodes,colnames_3cols){
  sample_path_2steps = sample_path[,5:ncol(sample_path)]
  # only where first column is a source node
  sample_path_2steps=sample_path_2steps[sample_path_2steps$x2 %in% source_nodes,]
  a<-sample_path_2steps[,1:3]; b<-sample_path_2steps[,3:5];
  colnames(a)=colnames_3cols; colnames(b)=colnames_3cols; 
  
  sample_path_SIF <- bind_rows(a,b); sample_path_SIF<-sample_path_SIF[!duplicated(sample_path_SIF),];
  sample_path_SIF <- sample_path_SIF[!colSums(apply(sample_path_SIF, 1, is.na))>0,]; 
  sample_path_SIF<-sample_path_SIF[order(sample_path_SIF$target),]
  sample_path_SIF
}